...
router.get("/:id", optionalAuth, async (req, res) => {
  try {
    ...
    if (content.status === "draft") {
      if (
        !req.user ||
        (req.user.id !== content.author_id && req.user.role !== "admin")
      ) {
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.role !== "admin" && existing.author_id !== req.user.id) {
        return res
          .status(403)
          .json({ error: "You can only edit your own content" });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.role !== "admin" && existing.author_id !== req.user.id) {
        return res
          .status(403)
          .json({ error: "You can only delete your own content" });
      }
